...
import numpy as np
import time
from config import USE_GPU, TRAIN_RATIO, VAL_RATIO
from utils.persistence import save_py_obj
from utils.visualization import draw_line_graph_by_list
from utils.visualization import draw_feature_map

class NormTrainer:
    """通用训练器，用于有输入，输出的场景"""
    def __init__(self, model, dataset, criterion, eval_func, n_epoch, n_batch=20):
        self.model = model
        if USE_GPU:
            self.model.cuda()
        ...
        train_size = int(TRAIN_RATIO * len(dataset))  # 给定一个训练比例TRAIN_RATIO
